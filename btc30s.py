...
def save_daily(date_str, candles, last_close):
    ...
    day_start_ts = int(
        day_dt.replace(hour=0, minute=0, second=0, microsecond=0).timestamp() * 1000
    )
...
def merge_daily_files_and_compute_indicators():
    ...
    if not df_list:
        logger.warning("Không có file daily hợp lệ để gộp.")
        return
    ...
    try:
        ...
        if resume_date > start_date.date():
            ...
            if os.path.exists(prev_file) and is_daily_file_valid(prev_file):
                try:
                    with open(prev_file, "r", newline="") as f:
